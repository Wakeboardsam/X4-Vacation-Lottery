...
test('Rules: Content Key duplicate checking works correctly', () => {
    ...
    const headers = rt.getDataRange().getValues()[0];
    const contentKeyCol = headers.indexOf('Content Key');
    assert.ok(contentKeyCol >= 0, 'Content Key column should exist');

    const duplicateKey = 'VACATION_ROUND_1_ORDER';
    const numCols = headers.length;

    const newRow = new Array(numCols).fill('');
    newRow[contentKeyCol] = duplicateKey;
    rt.appendRow(newRow);

    const res = initializeOrUpdateWorkbook();
    assert.equal(res.ok, false);
    assert.match(res.message, /Initialization aborted due to conflicts/);
    assert.equal(res.data.report.conflicts.some(c => c.includes('Duplicate nonblank Content Keys') && c.includes(duplicateKey)), true);
});

test('Rules: String FALSE parses as false', () => {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const tm = ss.getSheetByName('Turn Management');
    tm.appendRow(['Participant 1', 'pid1', '0000', '', true, 1, 1, true, 1, true, 1, 'FALSE', 'NONE', 'NONE', '2024']);

    // Test parsing string FALSE
    const state = getParticipantAcknowledgmentState('pid1', '2024');
    assert.equal(state.holidayVolunteer, false);
});
