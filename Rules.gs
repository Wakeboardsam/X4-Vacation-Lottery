...
function parseRulesBoolean_(value) {
...
function getParticipantAcknowledgmentState_(participantId, activeYear) {
  ...
  'Transfer Giver',
  'Transfer Receiver',
  'Rules Acknowledged Year'
];

for (const header of requiredHeaders) {
  if (map[header] === undefined) return null;
}
    ...
    const ackYear = String(data[matchRowIdx][map['Rules Acknowledged Year']] || '').trim();
    return {
        acknowledgedYear: ackYear,
        acknowledged: String(ackYear).trim() === String(activeYear).trim(),
        holidayVolunteer: parseRulesBoolean_(data[matchRowIdx][map['Holiday Volunteer']]),
        transferGiver: parseRulesBoolean_(data[matchRowIdx][map['Transfer Giver']]),
        transferReceiver: parseRulesBoolean_(data[matchRowIdx][map['Transfer Receiver']])
    };
}
...
function saveRulesAcknowledgment_(token, holidayVolunteerAnswer, transferPreference) {
    ...
    try {
        ...
        if (!/^\d{4}$/.test(activeYear)) return _rApi(false, null, 'Active Year is not configured correctly.');
